(herald "GSKE, part of Fluffy: Simplified Key Exchange for Constrained Environments draft-hardjono-ace-fluffy-03"
 (comment "Proposed fixes to the Internet-Draft: https://www.ietf.org/archive/id/draft-hardjono-ace-fluffy-03.txt"))
...
(defprotocol fluffy basic
 ...
 (defrole keyserv
  ...
  (trace
   (recv (cat "req" s g (enc (hash (cat "req" s g)) b nb (ltk s b))))
   (send (cat "resp" b (enc s g nb gk (ltk s b))))
   (recv (cat "fetch" s g (enc (hash (cat "fetch" s g)) a na (ltk s a))))
